...
function fmt(n: number): string {
  ...
  if (Number.isInteger(r)) return r.toLocaleString('es-ES');
...
export default function CalculadoraPrescripcion() {
  ...
  const caldoRango = (esSpray && cultivo) ? CALDO_RECOMENDADO[cultivo][tipoPlaga] : null;
  ...
  const infoFamilia: Record<string, string> = {
    hl:    'Dosis por cada 100 L de agua (caldo). Necesitas indicar el cultivo para obtener el caldo recomendado.',
    pct:   'Porcentaje de producto en la mezcla (% v/v). Ej: 0,1 % = 100 mL por cada 100 L de agua.',
    ha:    'Dosis por hectárea. Se calcularán los totales directamente.',
    m2:    'Dosis por metro cuadrado. Indica la superficie total a tratar en m².',
    m3:    'Dosis por metro cúbico. Indica el volumen de la cámara o invernadero a fumigar.',
    porkg: 'Dosis por kg de material tratado. Indica el peso total del producto a tratar (semillas, sustrato…).',
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-900">
        Elige la <strong>unidad que aparece en la etiqueta</strong> e introduce los datos de la parcela
        para obtener la cantidad práctica que el agricultor puede medir y aplicar.
      </div>

      {/* ─── Sección 1: Dosis en etiqueta ─── */}
      ...
      {/* ─── Sección 2: Cantidad (campo dinámico según familia) ─── */}
      <div className="space-y-2">
      ...
      {/* ─── Sección 3: Cultivo y plaga (solo para spray) ─── */}
      {esSpray && (
      ...
      {resultado && (
        <div className="space-y-4 border-t border-gray-100 pt-5">
          ...
          {/* Dosis por hectárea — solo para familias spray */}
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">
              {resultado.familia === 'm2'
                ? `Plan para ${fmt(resultado.cantidad)} m²`
                : resultado.familia === 'm3'
                ? `Plan para ${fmt(resultado.cantidad)} m³`
                : resultado.familia === 'porkg'
                ? `Plan para ${fmt(resultado.cantidad)} kg de material`
                : `Plan para ${fmt(resultado.cantidad)} ha`}
            </p>

            {resultado.totalCaldo !== undefined && resultado.numCubas !== undefined && resultado.productoPorCubaValor !== undefined ? (
              <div className="grid grid-cols-2 gap-3">
                <MetricCard label="Total de caldo"         valor={fmt(resultado.totalCaldo)}             unidad="L"                                   color="blue"   />
                <MetricCard label={`Cubas (${resultado.cubaCapacidad} L)`} valor={String(resultado.numCubas)} unidad="llenados"                       color="violet" />
                <MetricCard label="Producto / cuba"        valor={fmt(resultado.productoPorCubaValor)}   unidad={resultado.productoPorCubaUnidad ?? ''} color="amber"  />
                <MetricCard label="Total producto"         valor={fmt(resultado.totalProductoValor)}      unidad={resultado.totalProductoUnidad}        color="green"  />
              </div>
            ) : (
              <div className="space-y-3">
                <MetricCard
                  label={resultado.familia === 'm2' ? 'Total producto para tu superficie'
                       : resultado.familia === 'm3' ? 'Total producto para el volumen'
                       : resultado.familia === 'porkg' ? 'Total producto para el peso indicado'
                       : 'Total producto'}
                  valor={fmt(resultado.totalProductoValor)}
                  unidad={resultado.totalProductoUnidad}
                  color="green"
                />
                {esSpray && !resultado.caldoRango && (
